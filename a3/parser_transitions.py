...
def minibatch_parse(sentences, model, batch_size):
    ...
    while i1<len(partial_parses):

        mini_batch = partial_parses[i1:i2]

        while len(mini_batch) > 0:
            transitions = model.predict(mini_batch)
            
            for i,t in enumerate(transitions):
                mini_batch[i].parse_step(t)
            mini_batch = [pp for pp in mini_batch if len(pp.stack)>1 or len(pp.buffer)>0]
        
        i1 = i2
        i2 += batch_size
        if i2 > len(partial_parses):
            i2 = -1
