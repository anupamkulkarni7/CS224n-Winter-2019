...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        src_embed= self.model_embeddings.source(source_padded)
        X = pack_padded_sequence(src_embed,source_lengths)

        enc_hiddens,(last_hidden,last_cell) = self.encoder(X)
        enc_hiddens,_ = pad_packed_sequence(enc_hiddens,batch_first=True,total_length=source_padded.size(0))
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_h_transpose = torch.unsqueeze(dec_hidden,2)
        
        e_t = torch.bmm(enc_hiddens_proj,dec_h_transpose)
